...
public class Clouds {
    public CloudStorage alphabeticallyFirst(List<CloudStorage> input){
        return Collections.min(input, Comparator.comparing(CloudStorage::getProvider, String::compareToIgnoreCase));
    }

    public CloudStorage bestPriceForShortestPeriod(List<CloudStorage> input){
        return Collections.min(input, Comparator.comparing(CloudStorage::getPeriod,
